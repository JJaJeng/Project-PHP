...
?>
    <h1 class="text-center">
        <?php echo "แบบฟอร์มแก้ไขข้อมูลพนักงาน" ?>
    </h1>
            ...
            <div class="form-group">
                ...
                <select name="department_id" class="form-control">
                    <?php while($row=$result->fetch(PDO::FETCH_ASSOC)){?>  
                        <option <?php if($row["department_id"] == $emp["department_id"]) echo "selected" ?>
                        value="<?php echo $row["department_id"]?>"><?php echo $row["department_name"] ?></option>
                    <?php }?>
                </select>
            </div>
            <input type="submit" name="submit" value="อับเดต" class="btn btn-primary my-3">
        </form>
    </div>
</body>
</html>
